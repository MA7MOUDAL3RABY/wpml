...
add_filter(
	...
	function( $pre, $parsed_args, $url ) {
		if ( false !== strpos( $url, 'https://api.wpml.org/' ) && 'site_key_validation' === $parsed_args['body']['action'] ) {
			$obj                                       = new stdClass();
			$obj->subscription_data                    = new stdClass();
			$obj->subscription_data->status            = 4;
			$obj->subscription_data->expires           = null;
			$obj->subscription_data->subscription_type = 3;
			return array(
				'response' => array(
					'code'    => 200,
					'message' => 'ОК',
